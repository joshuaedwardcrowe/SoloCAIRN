...
**Calibrating.** Points are relative, so the first estimation session has nothing to compare against. Rather than sizing stories one at a time in isolation, order the batch against each other first, then anchor the scale by agreeing two well-understood reference stories — say a two and a five. Two anchors rather than one matters: pinning everything to a single smallest story biases later estimates downward, and leaves nothing underneath it without resorting to fractions. From then on, estimate by triangulation — compare each new story against two already-estimated ones, ideally one smaller and one larger ("more than that three, less than that eight"). Triangulation is the estimate, not a check applied after a number has been picked; planning poker is how a team surfaces disagreement about where a story sits between its neighbours. Keep the reference stories visible during refinement.
